...
export default function HomeHeader({sections, activeLink, setActiveLink, toggleSidebar }: HomeHeaderProps) {
  ...
  const ThemeIcon = mounted ? (theme === 'dark' ? Moon : Sun) : null
  ...
  return (
    <header
      className={`fixed z-30 top-0 w-full bg-gradient-to-b from-background via-transparent to-transparent h-14 transition-all duration-300 ${
        !isScrolling ? 'backdrop-blur-lg supports-[backdrop-filter]:bg-background/80' : ''
      }`}
    >
      <div id="" className="w-full h-full px-4 md:px-12 lg:px-16 flex items-center justify-between bg-transparent">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Link
              href={userInfo ? `/${userInfo?.role === 1 ? "admin" : "user"}/` : "/signin"}
            >
              {userInfo ? (
                <>
