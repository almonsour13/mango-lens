        ...
        return (
          <g key={index}>
            <rect
              ...
              className={`${color}`}
              strokeWidth="2"
              onMouseEnter={() => setHoveredDisease(box.diseaseName)}
  ...
  return (
    <div className="relative md:max-w-80 md:max-h-80 flex-1 aspect-square min-w-80 rounded-lg overflow-hidden shadow-lg cursor-pointer group ">
      <div className="relative w-full aspect-square">
        {(label === "Original" && originalImage) || (label === "Analyzed Image" && analyzedImage) ? (
        <Image
          src={label === "Analyzed Image" ? (showOriginal ? originalImage ?? "" : analyzedImage ?? "") : originalImage ?? ""}
        ...
        />
            ...
            <div className="flex flex-col items-center justify-center h-full bg-card">
              ...
              <p className="text-gray-400 text-sm">Analysis processed from previous version</p>
            </div>
        )}
        {(label === "Analyzed Image" && showOriginal && boundingBoxes?.length) && renderBoundingBoxes()}
        {label === "Analyzed Image"  && analyzedImage && boundingBoxes?.length && (
