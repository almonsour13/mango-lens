...
import { image$, analysis$, analyzedImage$, diseaseIdentified$, tree$ } from "./stores"
import type { Image as Img, ImageAnalysisDetails, ScanResult, TfJsDisease } from "@/types/types"
...
export function getImageByTreeID(treeID: string):images[] {
    const images = Object.values(observable(image$).get() || {}).filter((image) => image.treeID === treeID);
...
export function getImagesByUserID(userID: string) {
  const images = Object.values(image$.get() || {}).filter((image) => image.userID == userID)
  return images.map((image) => {
    const analysis = Object.values(analysis$.get() || {}).filter((analysis) => analysis.imageID === image.imageID)[0]
